...
MENU, MARKET, WALLET, RULES, FAQ_MENU, CHANNEL_UPDATE, OPEN_A_TICKET,CANADAPOST,DSS_VERIFICATION, POST_PREPAID,MAIL_FORWARDING = range(11)
POST_PREPAID_VERIFICATION = 11
def start(update: Update, context: CallbackContext) -> int:
    reply_markup = ReplyKeyboardMarkup([["Market", "Wallet"], ["Rules", "F.A.Q"], ["Channel Update", "Open a Ticket"]], one_time_keyboard=True)
...
def market(update: Update, context: CallbackContext) -> int:
    reply_markup = ReplyKeyboardMarkup([["Bank Drops", "Bank Logs"], ["Canada Post Services", "CVV/Fullz"], ["Documents & IDs", "Exchange Fiat for Crypto"],
                                        ["Hacked Accounts", "Profiles"], ["Scam Pages", "Templates"], ["Tutorials", "Back"]], one_time_keyboard=True)
...
def bank_drops(update: Update, context: CallbackContext) -> int:
    reply_markup = ReplyKeyboardMarkup([["DSS Verification", "Canada Post Prepaid", "Mail Forwarding", "Back"]], one_time_keyboard=True)
    update.message.reply_text('Select an option:', reply_markup=reply_markup)
...
def start_dss_verification(update: Update, context: CallbackContext) -> int:
    context.user_data['verification_data'] = {'first_name': None, 'last_name': None, 'dob': None, 'address': None, 'sex': None, 'dss_number': None}
...
def get_post_prepaid(update: Update, context: CallbackContext) -> int:
    ...
    if verification_data['first_name'] is None:
        ...
        update.message.reply_text(f"Email Address set to: {verification_data['email_address']}\nWhat is your Phone Number?")
        return POST_PREPAID_VERIFICATION
    elif verification_data['phone'] is None:
        verification_data['phone'] = update.message.text
        update.message.reply_text(f"Phone Number set to: {verification_data['phone']}\nWhat is your Address (Street, City, Province, Postal Code)?")
        return POST_PREPAID_VERIFICATION
    elif verification_data['address'] is None:
        verification_data['address'] = update.message.text
        update.message.reply_text(f"Address set to: {verification_data['address']}\nWhat is your Sex (M/F/Other)?")
    ...
    conversation_handler = ConversationHandler(
        ...
        states={
            ...
            MARKET: [MessageHandler(Filters.regex('^(Bank Drops)$'), bank_drops),
                   ...
                   MessageHandler(Filters.regex('^(CVV/Fullz)$'), faq_menu),
            ...
            WALLET: [MessageHandler(Filters.regex('^(Add Balance|View Balance|Order History|Usage History|Back)$'), back_to_menu)],
            ...
            CHANNEL_UPDATE: [MessageHandler(Filters.regex('^(Proof of Identity|Document Identity|Custom|Back)$'), back_to_menu)],
            OPEN_A_TICKET: [MessageHandler(Filters.regex('^(Proof of Identity|Document Identity|Custom|Back)$'), back_to_menu)],
            CANADAPOST: [MessageHandler(Filters.regex('^(DSS Verification|Canada Post Prepaid|Mail Forwarding|Back)$'), start_dss_verification)],
